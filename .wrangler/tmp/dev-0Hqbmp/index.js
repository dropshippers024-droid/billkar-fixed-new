...
// .wrangler/tmp/bundle-to7xb6/checked-fetch.js
...
app.get("/api/health", (c) => c.json({ status: "ok", message: "BillKaro API running" }));
...
app.post("/api/auth/login", async (c) => {
  ...
  const user = await c.env.DB.prepare(
    "SELECT id, email, full_name, phone, avatar_url FROM users WHERE email = ? AND password_hash = ?"
...
app.get("/api/auth/google", (c) => {
  const clientId = c.env.GOOGLE_CLIENT_ID || "99456824359-62hc724e8ovd24kqkmcreot9be1p60p4.apps.googleusercontent.com";
...
app.get("/api/auth/google/callback", async (c) => {
  ...
  const clientId = c.env.GOOGLE_CLIENT_ID || "99456824359-62hc724e8ovd24kqkmcreot9be1p60p4.apps.googleusercontent.com";
  const clientSecret = c.env.GOOGLE_CLIENT_SECRET || "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";
  const redirectUri = c.env.GOOGLE_REDIRECT_URI || "http://localhost:8787/api/auth/google/callback";
  console.log("[Google OAuth] Using client_id:", clientId);
  ...
  console.log("[Google OAuth] Token exchange response:", JSON.stringify(tokenData));
  ...
  let user = await c.env.DB.prepare(
    "SELECT id, email, full_name, phone, avatar_url FROM users WHERE email = ?"
  ).bind(email).first();
  if (!user) {
    const userId = crypto.randomUUID();
    const businessId = crypto.randomUUID();
    ...
    user = { id: userId, email, full_name: fullName, phone: "", avatar_url: googleUser.picture || "" };
  }
  const token = await createJWT(user.id, c.env.JWT_SECRET);
  const userJson = encodeURIComponent(JSON.stringify(user));
  return c.redirect(`http://localhost:8087/auth/callback?token=${token}&user=${userJson}`);
});
...
app.post("/api/invoices", authMiddleware, async (c) => {
  ...
  await c.env.DB.prepare(`
    INSERT INTO invoices (id, business_id, customer_id, customer_name, invoice_number,
      ...
      cgst, sgst, igst, total_amount, amount_paid, notes, terms, template_id, is_inter_state)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    invoiceId,
    businessId,
    ...
    inv.notes ?? "",
    inv.terms ?? "",
    inv.template_id ?? "modern",
    inv.is_inter_state ? 1 : 0
  ).run();
  ...
  if (inv.customer_name) {
    const existing = await c.env.DB.prepare(
      "SELECT id FROM customers WHERE business_id = ? AND LOWER(name) = LOWER(?)"
    ).bind(businessId, inv.customer_name.trim()).first();
    if (!existing) {
      await c.env.DB.prepare(
        "INSERT INTO customers (id, business_id, name, email, phone, gstin, state) VALUES (?, ?, ?, ?, ?, ?, ?)"
      ).bind(
        crypto.randomUUID(),
        businessId,
  ...
  return c.json({ id: invoiceId, invoice_number: inv.invoice_number }, 201);
});
app.put("/api/invoices/:id", authMiddleware, async (c) => {
  const businessId = c.get("businessId");
  const id = c.req.param("id");
  const updates = await c.req.json();
  const allowed = ["status", "amount_paid", "notes", "terms", "total_amount", "subtotal", "cgst", "sgst", "igst", "due_date"];
  const sets = [];
  const vals = [];
...
app.post("/api/customers", authMiddleware, async (c) => {
  ...
  return c.json({ id, name: cust.name }, 201);
});
...
app.post("/api/products", authMiddleware, async (c) => {
  ...
  return c.json({ id, name: prod.name }, 201);
});
...
app.get("/api/reports/sales", authMiddleware, async (c) => {
  ...
  const invoices = await c.env.DB.prepare(
    "SELECT * FROM invoices WHERE business_id = ? ORDER BY invoice_date DESC"
  ).bind(businessId).all();
  const expenses = await c.env.DB.prepare(
    "SELECT * FROM expenses WHERE business_id = ? ORDER BY date DESC"
  ).bind(businessId).all();
  const totalRevenue = invoices.results.reduce((s, i) => s + (i.total_amount || 0), 0);
  const totalExpenses = expenses.results.reduce((s, e) => s + (e.amount || 0), 0);
  return c.json({
...
// .wrangler/tmp/bundle-to7xb6/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-to7xb6/middleware-loader.entry.ts
