...
// .wrangler/tmp/bundle-scyOaV/checked-fetch.js
...
// .wrangler/tmp/bundle-scyOaV/strip-cf-connecting-ip-header.js
...
app.post("/api/invoices", authMiddleware, async (c) => {
  ...
  if (!businessId)
    return c.json({ error: "No business found" }, 404);
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot create invoices" }, 403);
...
app.put("/api/invoices/:id", authMiddleware, async (c) => {
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot update invoices" }, 403);
  const businessId = c.get("businessId");
  if (!businessId)
    return c.json({ error: "No business found" }, 404);
...
app.get("/api/recurring-invoices", authMiddleware, async (c) => {
  const businessId = c.get("businessId");
  if (!businessId)
    return c.json({ recurring_invoices: [], due_count: 0 });
...
app.post("/api/recurring-invoices/generate-due", authMiddleware, async (c) => {
  ...
  if (!businessId)
    return c.json({ error: "No business found" }, 404);
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot generate recurring invoices" }, 403);
...
app.put("/api/recurring-invoices/:id", authMiddleware, async (c) => {
  ...
  if (!businessId)
    return c.json({ error: "No business found" }, 404);
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot update recurring invoices" }, 403);
  await ensureRecurringInvoicesTable(c.env.DB);
  const id = c.req.param("id");
  ...
  if (updates.frequency !== void 0 && !frequency)
...
app.delete("/api/recurring-invoices/:id", authMiddleware, async (c) => {
  ...
  if (!businessId)
    return c.json({ error: "No business found" }, 404);
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot delete recurring invoices" }, 403);
  await ensureRecurringInvoicesTable(c.env.DB);
  const id = c.req.param("id");
  await c.env.DB.prepare(
...
app.post("/api/customers", authMiddleware, async (c) => {
  ...
  if (!businessId)
    return c.json({ error: "No business found" }, 404);
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot create customers" }, 403);
...
app.put("/api/customers/:id", authMiddleware, async (c) => {
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot update customers" }, 403);
  const businessId = c.get("businessId");
  if (!businessId)
    return c.json({ error: "No business found" }, 404);
  const id = c.req.param("id");
...
app.delete("/api/customers/:id", authMiddleware, async (c) => {
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot delete customers" }, 403);
  const businessId = c.get("businessId");
  const id = c.req.param("id");
...
app.post("/api/products", authMiddleware, async (c) => {
  ...
  if (!businessId)
    return c.json({ error: "No business found" }, 404);
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot create products" }, 403);
...
app.put("/api/products/:id", authMiddleware, async (c) => {
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot update products" }, 403);
  const businessId = c.get("businessId");
  if (!businessId)
    return c.json({ error: "No business found" }, 404);
  const id = c.req.param("id");
...
app.delete("/api/products/:id", authMiddleware, async (c) => {
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot delete products" }, 403);
  const businessId = c.get("businessId");
  const id = c.req.param("id");
...
app.post("/api/expenses", authMiddleware, async (c) => {
  ...
  if (!businessId)
    return c.json({ error: "No business found" }, 404);
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot create expenses" }, 403);
...
app.put("/api/expenses/:id", authMiddleware, async (c) => {
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot update expenses" }, 403);
  const businessId = c.get("businessId");
  if (!businessId)
    return c.json({ error: "No business found" }, 404);
  const id = c.req.param("id");
...
app.delete("/api/expenses/:id", authMiddleware, async (c) => {
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot delete expenses" }, 403);
  const businessId = c.get("businessId");
  const id = c.req.param("id");
...
app.post("/api/payments", authMiddleware, async (c) => {
  if (c.get("teamRole") === "viewer")
    return c.json({ error: "Viewers cannot record payments" }, 403);
  const businessId = c.get("businessId");
  if (!businessId)
    return c.json({ error: "No business found" }, 404);
  ...
  if (!invoiceId)
    return c.json({ error: "Invoice is required" }, 400);
  if (amount <= 0)
...
app.get("/api/reports/gst", authMiddleware, async (c) => {
  ...
  if (!month)
    return c.json({ error: "month query param required (e.g. 2026-03)" }, 400);
...
app.put("/api/team/:id", authMiddleware, async (c) => {
  ...
  if (!normalizedRole)
    return c.json({ error: "Invalid team role" }, 400);
  await c.env.DB.prepare(
...
// workers/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// workers/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-scyOaV/middleware-insertion-facade.js
...
// workers/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-scyOaV/middleware-loader.entry.ts
