...
async function upsertCustomerFromInvoiceInput(
  db: D1Database,
  businessId: string,
  ...
  let customer =
    requestedCustomerId
      ...
      ).bind(requestedCustomerId, businessId).first<CustomerRow>()
      : null
  ...
  if (!customer && requestedCustomerName) {
    ...
    ).bind(businessId, requestedCustomerName).first<CustomerRow>()
  }
...
async function fetchScopedInvoice(db: D1Database, invoiceId: string, businessId: string): Promise<Record<string, unknown> | null> {
  ...
  `).bind(invoiceId, businessId).first<Record<string, unknown>>()
}
...
async function generateManagedInvoiceNumber(db: D1Database, businessId: string): Promise<string> {
  ...
  ).bind(businessId).first<BusinessSequenceRow>()
...
function buildRecurringInvoiceSnapshot(
  invoiceInput: Record<string, unknown>,
...
async function upsertRecurringInvoice(
  db: D1Database,
  businessId: string,
  ...
  ).bind(sourceInvoiceId, businessId).first<Pick<RecurringInvoiceRow, 'id'>>()
...
async function generateDueRecurringInvoices(
  ...
  `).bind(businessId, today).all<RecurringInvoiceRow>()
...
async function authMiddleware(c: AppContext, next: () => Promise<void>) {
  ...
  // Support X-Business-Id header for business context switching (team members)
  const headerBizId = c.req.header('X-Business-Id')
  if (headerBizId) {
    // Verify user has access: either owns this business or is an active team member
    ...
    if (ownsBiz) {
      ...
      ).bind(headerBizId, userId).first<TeamAccessRow>()
      if (teamAccess) {
        c.set('businessId', headerBizId)
    ...
    if (biz) {
      c.set('businessId', biz.id as string)
    }
  }
...
app.post('/api/auth/forgot-password', async (c) => {
  ...
  ).bind(normalizedEmail).first<Pick<UserRow, 'id' | 'email'>>()
...
app.post('/api/auth/reset-password', async (c) => {
  ...
  ).bind(tokenHash).first<PasswordResetRow>()
...
app.post('/api/auth/change-password', authMiddleware, async (c) => {
  ...
  ).bind(userId).first<Pick<UserRow, 'id' | 'password_hash'>>()
...
app.get('/api/auth/google/callback', async (c) => {
  ...
  if (!user) {
    ...
    // Server-side expiry check for existing Google users
    if (user.plan && user.plan !== 'free' && user.plan_expires_at && new Date(user.plan_expires_at) < new Date()) {
      user.plan = 'free'
      user.plan_expires_at = null
      await c.env.DB.prepare('UPDATE users SET plan = ?, plan_expires_at = NULL WHERE id = ?').bind('free', user.id).run()
...
app.get('/api/business', authMiddleware, async (c) => {
  ...
  // Fallback: get user's own business
  const userId = c.get('userId')
...
app.put('/api/business', authMiddleware, async (c) => {
  ...
  await c.env.DB.prepare(`
    ...
    WHERE id = ?
  `).bind(
    b.name ?? null, b.gstin ?? null, b.pan ?? null,
    b.email ?? null, b.phone ?? null, b.address ?? null,
    b.city ?? null, b.state ?? null, b.pincode ?? null,
    b.logo_url ?? null, b.signature_url ?? null,
    b.bank_name ?? null, b.account_number ?? null,
    b.ifsc ?? null, b.upi_id ?? null,
    b.invoice_prefix ?? null, b.next_invoice_number ?? null,
    businessId
  ).run()
...
app.post('/api/invoices', authMiddleware, async (c) => {
  ...
  const balanceDue = inv.balance_due !== undefined
    ? toFiniteNumber(inv.balance_due, Math.max(totalAmount - amountPaid, 0))
  ...
  await c.env.DB.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    invoiceId, businessId, customerId, customerName,
    getTrimmedString(inv.invoice_number), getOptionalString(inv.invoice_date), getOptionalString(inv.due_date),
    getTrimmedString(inv.type) || 'invoice', getTrimmedString(inv.status) || 'draft',
    subtotal, taxableAmount,
    cgst, sgst, igst,
    totalAmount, amountPaid, balanceDue,
    getTrimmedString(inv.notes), getTrimmedString(inv.terms), getTrimmedString(inv.template_id) || 'modern',
    toBooleanInt(inv.is_inter_state)
  ).run()

  await replaceInvoiceItems(c.env.DB, invoiceId, inv.items)
  if (inv.recurring !== undefined) {
...
app.put('/api/invoices/:id', authMiddleware, async (c) => {
  ...
  ).bind(id, scopedBusinessId).first<Pick<InvoiceRow, 'id'>>()
  ...
  if (updates.customer_id !== undefined || updates.customer_name !== undefined || customerId) {
  ...
  if (sets.length === 0 && updates.items === undefined && updates.recurring === undefined) {
...
app.get('/api/recurring-invoices', authMiddleware, async (c) => {
  ...
  ).bind(businessId).all<RecurringInvoiceRow>()

  const recurringInvoices = (results || []).map((row) => ({
    ...row,
    auto_send: Boolean(row.auto_send),
    active: Boolean(row.active),
    invoice_data: safeJsonParse<Record<string, unknown>>(row.invoice_data, {}),
  }))
  const dueCount = recurringInvoices.filter((row) =>
    row.active &&
    typeof row.next_date === 'string' &&
    row.next_date <= today &&
    (!row.end_date || row.next_date <= row.end_date)
  ).length
...
app.put('/api/recurring-invoices/:id', authMiddleware, async (c) => {
  ...
  ).bind(id, businessId).first<RecurringInvoiceRow>()
  ...
  return c.json({
    recurring_invoice: recurringInvoice
      ? {
        ...
        invoice_data: safeJsonParse<Record<string, unknown>>(recurringInvoice.invoice_data, {}),
      }
      : null,
  })
})
...
app.post('/api/customers', authMiddleware, async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    id, businessId, cust.name ?? '', cust.gstin ?? '', cust.phone ?? '',
    cust.email ?? '', cust.billing_address ?? '', cust.shipping_address ?? '', cust.state ?? ''
  ).run()
...
app.put('/api/customers/:id', authMiddleware, async (c) => {
  ...
  await c.env.DB.prepare(`
    ...
    WHERE id = ? AND business_id = ?
  `).bind(
    cust.name ?? null, cust.gstin ?? null, cust.phone ?? null,
    cust.email ?? null, cust.billing_address ?? null,
    cust.shipping_address ?? null, cust.state ?? null,
    id, businessId
  ).run()
...
app.post('/api/products', authMiddleware, async (c) => {
  ...
  await c.env.DB.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    id, businessId, prod.name ?? '', prod.hsn_sac_code ?? '',
    prod.type ?? 'goods', prod.unit ?? 'pcs',
    prod.selling_price ?? 0, prod.purchase_price ?? 0,
    prod.gst_rate ?? 18, prod.stock_quantity ?? 0, prod.low_stock_threshold ?? 10
  ).run()
...
app.put('/api/products/:id', authMiddleware, async (c) => {
  ...
  await c.env.DB.prepare(`
    ...
    WHERE id = ? AND business_id = ?
  `).bind(
    prod.name ?? null, prod.hsn_sac_code ?? null,
    prod.type ?? null, prod.unit ?? null,
    prod.selling_price ?? null, prod.purchase_price ?? null,
    prod.gst_rate ?? null, prod.stock_quantity ?? null,
...
app.post('/api/expenses', authMiddleware, async (c) => {
  ...
  await c.env.DB.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    id, businessId, exp.category ?? '', exp.description ?? '',
    exp.amount ?? 0, exp.gst_amount ?? 0, exp.date ?? null,
    exp.vendor_name ?? '', exp.receipt_url ?? '', exp.payment_method ?? 'cash'
  ).run()
...
app.post('/api/payments', authMiddleware, async (c) => {
  ...
  const invoice = await c.env.DB.prepare(
    'SELECT id FROM invoices WHERE id = ? AND business_id = ?'
  ).bind(invoiceId, businessId).first<Pick<InvoiceRow, 'id'>>()
  ...
  `).bind(
    id, invoiceId, amount,
    getOptionalString(pay.payment_date), getTrimmedString(pay.payment_method) || 'cash',
    getTrimmedString(pay.reference_number), getTrimmedString(pay.notes)
  ).run()
...
app.get('/api/reports/gst', authMiddleware, async (c) => {
  ...
  const endDate = `${month}-31` // SQLite BETWEEN works fine with this
...
// GET /api/team/invite/:id — Get invite details (public, no auth needed)
...
// POST /api/team/accept/:id — Accept invite (requires auth)
...
app.post('/api/team/invite', authMiddleware, async (c) => {
  ...
  ).bind(businessId).first<CountRow>()

  const currentCount = (teamCount?.count || 0) + 1 // +1 for owner
  ...
  ).bind(normalizedEmail).first<Pick<UserRow, 'id'>>()
...
app.get('/api/admin/stats', async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      freeUsers: (totalUsers?.count as number || 0) - (proUsers?.count as number || 0) - (businessUsers?.count as number || 0),
