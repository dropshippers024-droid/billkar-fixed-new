...
import { Eye, EyeOff, Loader2, FileText, Shield, Zap, Send } from "lucide-react";
import { signUp } from "@/lib/authStore";
import { toast } from "sonner";

const features = [
  { icon: Zap, text: "Create invoices in under 30 seconds" },
  { icon: Shield, text: "Auto GST calculation — CGST, SGST, IGST" },
  { icon: Send, text: "PDF, WhatsApp, Email — share anywhere" },
];
...
const Signup = () => {
  ...
  const strength =
    password.length === 0
      ...
      ? 4
      : 3;
  const strengthColors = ["bg-gray-200", "bg-red-500", "bg-yellow-500", "bg-amber-400", "bg-emerald-500"];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(inviteId ? `/invite/${inviteId}` : "/onboarding");
    } catch (err) {
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-12 bg-white">
        ...
        >
          ...
          <p className="text-gray-500 text-sm mb-8">Start with 50 free invoices. No credit card needed.</p>
              ...
              {password.length > 0 && (
                <div className="flex gap-1 mt-2">
                  {[1, 2, 3, 4].map((s) => (
                    <div
                      key={s}
                      className={`h-1 flex-1 rounded-full transition-all duration-300 ${s <= strength ? strengthColors[strength] : "bg-gray-200"}`}
                    />
                  ))}
                </div>
              )}
        ...
        <div className="relative z-10 px-12 max-w-md">
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-white leading-tight max-w-sm">
              The simplest way to manage GST invoicing.
            </h2>
            <p className="text-gray-400 text-sm mt-3">
              Join 2,000+ Indian businesses.
            </p>
          </motion.div>
